...
def about(topic):
    ...
    def helper(args):
        ...
        for i in range(len(origin)):
            words.append(lower(remove_punctuation(origin[i])))
...
def accuracy(typed, source):
    ...
    count = 0
    length = len(typed_words) if len(typed_words) < len(source_words) else len(source_words)
...
def time_per_word(words, times_per_player):
    ...
    for player_times in times_per_player:
        word_time = []
        for i in range(len(words)):
            word_time.append(player_times[i + 1] - player_times[i])
