...
def distance(city_a, city_b):
    ...
    result = (lat_a - lat_b)**2 + (lon_a - lon_b)**2
    result = sqrt(result)
    return result
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    local_city = make_city("local", lat, lon)
    dis_a = distance(local_city, city_a)
    dis_b = distance(local_city, city_b)
    if dis_a < dis_b:
...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    ''' I have tried few ways, but all fail
    new_tree = []
    tree(new_tree, label(t))
    if is_leaf(t):
        return tree(new_tree, leaves)
    for b in branches:
        new_tree = sprout_leaves(b, leaves)
    return new_tree
    '''
    # chatgpt
    if is_leaf(t):
